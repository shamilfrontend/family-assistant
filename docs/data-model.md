...
- События календаря, открытые дела, здоровье и документы: «сегодня» и «скоро» (завтра … +7 дней) в поясе семьи. `HEALTH_APPOINTMENT` в список событий не входит — приёмы из здоровья. `remindInUi` на сводку не влияет.
...
- Документы в сводке: истекают сегодня … +7 дней. На экране документов индикатор `expiresSoon` — 30 дней (подфаза 1.4).
